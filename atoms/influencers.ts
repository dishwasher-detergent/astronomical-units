...
} from "@/constants/INFLUENCERS";

export const influencers = atomWithReducer(
  INFLUENCERS,
  (current) => current + INFLUENCERS_DELTA
);
...
export const influencersRate = atom((get) => {
  let newRate =
    INFLUENCERS_RATE - get(influencersRateReduction) * INFLUENCERS_RATE_DELTA;
